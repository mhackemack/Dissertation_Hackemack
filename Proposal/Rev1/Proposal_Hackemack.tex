...
\vspace{-1.5mm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent\rule{7.00in}{0.4pt}

{\Large \bf Abstract}
\vspace{2mm}

Solutions of the neutral particle transport equation have many applications across the medical, energy, and industrial fields. The Discontinuous Galerkin Finite Element Method (DGFEM) is a common spatial discretization method to solve the multigroup discrete ordinates ($S_N$) equations. Traditionally, the DGFEM $S_N$ equations have been solved using linear trial spaces on simple meshes (triangles and quadrilaterals in 2D and tetrahedra and hexahedra in 3D), with recent extensions to higher-order trial spaces on triangular grids. There is two-fold benefit in realizing higher-order FEM trial spaces on aribtrary polytope (polygons and polyhedra) grids. Higher-order FEM basis functions are beneficial because they achieve enhanced convergence rates, while simultaneously improving parallel efficiency with a decrease in memory-access bottlenecks by giving the CPUs more data to compute per solve. Polytope meshes are beneficial because they remove limitations of simple mesh types and simultaneously bring us up-to-date with other physics and engineering communities.

This dissertation work seeks to marry the tasks of calculating higher-order transport solutions on arbitrary polytope meshes for use on massively parallel computer architectures. We will investigate different 2D linear polygonal basis functions for use with the transport equation. We then extend the analysis to the quadratic serendipity space of polygonal basis functions based on recent work by the applied mathematics community. We finish the dissertation work by analyzing the Modified Interior Penalty (MIP) diffusion form for use with diffusion synthetic acceleration (DSA) for massively-parallel transport problems on arbitrary grids. MIP DSA's scalability and efficacy for use as a preconditioner for thermal neutron upscattering problems will be investigated.
...
\noindent where the trace is defined as: $\Psi_m^{\pm} (\vec{r}) \equiv \lim_{s \rightarrow 0^{\pm}} \Psi_m \Big( \vec{r} + s (\vec{\Omega} \cdot \vec{n})  \vec{n}  \Big)$. We then insert the upwind notation of Eq. (\ref{eq::upwind_eqs}) into Eq. (\ref{eq::disc_trans_eq_cellK}) to yield the full set of spatially discretized equations for cell $K$,
...
At this point we note that we have not specified a mesh or basis function type. In finite element theory, the basis functions are typically collections of polynomial functions with measure on local elements within the domain \cite{ern2013theory}. A particular application of the Bramble-Hilbert lemma states that we can achieve error convergence rates between the true solution, $u$, and the discretized solution, $u_h$, by,

\begin{equation}
\label{eq::bf_conv_rate_h}
|| u - u_h ||_{L_2} = C h^{q}, \qquad q = \min(p+1, r)
\end{equation}

\noindent where $h$ is the maximum diameter of a mesh element, $p$ is the polynomial order of the basis functions, $C$ is a constant independent of the mesh, and $r$ is the regularity of the transport solution \cite{bramble1970estimation}. We can clearly see that if there is sufficient irregularity in the transport solution, we cannot achieve convergence rates greater than $r$. If $h$ is difficult to estimate for a particular mesh type, we can proportionally relate it to the number of spatial degrees of freedom in the problem: $N_{dof} \propto p h^{-d}$, where $d$ is the dimensionality of the problem (1,2,3). Using this relation, Eq. (\ref{eq::bf_conv_rate_h}) can be rewritten to be dependent on the number of spatial degrees of freedom,

\begin{equation}
\label{eq::bf_conv_rate_DOF}
|| u - u_h ||_{L_2} = C(p) N_{dof}^{- \frac{q}{d}},
\end{equation}

\noindent where $C(p)$ is now a functions of the polynomial order, $p$. As long as the number of spatial degrees of freedom does not grow proportionally to the polynomial order, we expect to achieve more accurate solution estimates in relation to the work required to solve the problem with higher-order interpolants.
...
For the DSA work in PDT, there are three items left to investigate. First, we need to complete the last of the numerical experiments to verify the theoretical results of the DSA fourier analysis. This is mostly restricted to the numerical verification of the PHI problem. Second, we are running additional scaling suites to better characterize MIP DSA timing in comparison to transport sweeps involving variable numbers of angles and energy groups. Finally, we need to conclude the remaining numerical experiments for the IM1 problem. At this point in time, these numerical experiments are now restricted to simply rerunning the IM1 problems with prismatic polyhedral grids.
...
\begin{enumerate}
	\item 2D polygonal transport calculations
	\begin{enumerate}
		\item Analyze the Wachspress, PWL, mean value, and maximum entropy 2D linear polygonal basis functions for use in DGFEM transport calculations
		\item Perform the same analysis with the quadratic serendipity extension of the 2D linear polygonal basis functions
		\item Perform convergence rate analysis on benchmark and analytical cases using polygonal meshes and AMR (we include cases where we are restricted by the irregularity in the transport solution)
	\end{enumerate}
	\item MIP DSA calculations on polytopees
	\begin{enumerate}
		\item Analyze the 2D linear and quadratic basis functions through Fourier and numerical analysis
		\item Extend the analysis of MIP DSA to 3D polyhedra through Fourier and numerical analysis
		\item Analyze the performance of MIP DSA on AMR problems using polygonal grids without hanging-node treatment
		\item Analyze the scalability of MIP in PDT to high processor counts
	\end{enumerate}
	\item Thermal neutron upscatter acceleration
	\begin{enumerate}
		\item Study the convergence properties of the presented upscatter acceleration methods
		\item Analyze the effects of strongly heterogeneous configurations
		\item Analyze the effects of parallel concurrency between the methods at scale
