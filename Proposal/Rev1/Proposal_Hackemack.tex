...
\noindent where the trace is defined as: $\Psi_m^{\pm} (\vec{r}) \equiv \lim_{s \rightarrow 0^{\pm}} \Psi_m \Big( \vec{r} + s  \vec{\Omega}_m  \Big)$. The trace value $s$ is negative within cell $K$ and positive outside cell $K$. We then insert the upwind notation of Eq. (\ref{eq::upwind_eqs}) into Eq. (\ref{eq::disc_trans_eq_cellK}) to yield the full set of spatially discretized equations for cell $K$,
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Analyze the performance of MIP DSA on AMR problems for polygonal grids without hanging nodes
