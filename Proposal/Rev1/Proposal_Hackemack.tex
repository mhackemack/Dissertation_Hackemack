...
For the generalized barycentric coordinates, we consider a polygon $K \in \mathbb{R}^2$ with $N_K$ vertices. The basis function corresponding to vertex $i$, $\lambda_i$, is strictly non-negative for any point $\vec{x}$ on the polygon $K$: $\lambda_i \geq 0$. The basis functions also satisfy the {\em Kronecker} property at the nodal points, 
...
\noindent The functional interpolation space of the generalized barycentric coordinates is such that the basis functions satisfy,
...
\noindent for the linear constraint. We do not provide the functional form for all of the basis functions here for brevity. However, we will give a general description of their properties along with their restrictions. The Wachspress coordinates 
