...
\noindent which is beholden to the following trace condition:

\begin{equation}
\label{eq::Sn_Spatial_Convergence_trace}
\Psi_m^{\pm} \equiv \lim\displaylimits_{s \rightarrow 0^\pm} \Psi (\vec{r} + s (\vec{\Omega}_m \cdot \vec{n} ) \, \vec{\Omega}_m ) .
\end{equation}
