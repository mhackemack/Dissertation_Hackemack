...
\begin{equation}
\label{eq::Sn_full_sol_gs_ops}
\begin{aligned}
{\bf L}_{gs} {\bf \Psi}_{gs} &=    {\bf M}_{gs} {\bf \Sigma}_{gs} {\bf \Phi}_{gs} +  {\bf Q}_{gs}  \\
{\bf \Phi}_{gs} &=  {\bf D}_{gs} {\bf \Psi}_{gs}
\end{aligned}
\end{equation}

\noindent where ${\bf L}_{gs}$ is the fully-discretized loss operator which consists of total interaction and streaming terms, ${\bf M}_{gs}$ is the moment-to-discrete operator of the angular discretization, ${\bf D}_{gs}$ is the discrete-to-moment operator of the angular discretization, ${\bf \Sigma}_{gs}$ is the scattering operator of the multigroup and angular discretizations, and ${\bf Q}_{gs}$ is the full phase-space distributed source. In this case, the source contains contributions from boundary and domain sources, fission sources, and scattering sources from outside the group set into the group set of interest. We note that ${\bf M}_{gs}$ and ${\bf D}_{gs}$ are also group set dependent since we do not enforce the exact same angular discretization on all group sets. This can be useful if certain ranges of energy groups require higher angular fidelity due to increased anisotropic scattering.



\begin{equation}
\label{eq::Sn_full_sol_moments_only}
\left( {\bf I} -{\bf T}_{gs} \right) {\bf \Phi}_{gs} =  {\bf D}_{gs} {\bf L}_{gs}^{-1}  {\bf Q}_{gs}
\end{equation}
