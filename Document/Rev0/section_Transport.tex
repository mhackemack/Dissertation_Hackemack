...
For the remainder of this energy discretization procedure, we will utilize the steady-state form of the transport equation in Eq. (\ref{eq::Sn_transport_eq_op_SS}). The time-dependent and eigenvalue forms are analagous and would be derived identically. Taking the energy interval for group $g$ as defined in Eq. (\ref{eq::MG_energy_interval}), the energy-integrated angular flux of group $g$ is

\begin{equation}
\label{eq::MG_ang_flux_g}
...
\begin{aligned}
	 \left( \vec{\Omega}_m \cdot \vec{\nabla}  + \sigma_{t,g} (\vec{r}) \right)  \Psi_{m,g} (\vec{r}) = \sum_{g'=1}^{G} \sum_{p=0}^{N_P} \frac{2p + 1}{4 \pi} \sigma_{s,p}^{g' \rightarrow g}  (\vec{r}) \sum_{n=-p}^{p}  \Phi_{p,n,g'}  (\vec{r})  Y_{p,n} (  \vec{\Omega}_m )  \\
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/sec_Sn/two_cells.eps}
\caption{Two cells of the spatial discretization with the connecting face normal $\vec{n}_1$ oriented from cell $K_1$ to cell $K_2$.}
\label{fig::MG_two_ref_cells}
\end{figure}
