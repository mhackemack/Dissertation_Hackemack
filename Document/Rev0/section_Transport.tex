...
We begin our description of the LS quadrature by analyzing the angular coordinate system 
...
We do this by taking Eq. (\ref{eq::Sn_DGFEM_trans_eq_cellK_complete}), performing another integration-by-parts on the streaming term, mulitplying by the angular weight, $w_m$, and summing over all elements and all angular directions. 
...
\item The iterative solver does need to build any matrices explicitly but only requires the action of ${\bf L}^{-1}$.
