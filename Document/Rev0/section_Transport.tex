...
For the spatial discretization of the problem domain, we simplify Eq. (\ref{eq::Sn_mg_sn_trans_eq}) into a single energy group and drop the fission term (it can be lumped into the external source term and will act similarly to the total interaction term)
...
\noindent Also, just like the elementary mass matrix, we can use the same spatial quadrature set, $\left\{  \vec{x}_q , w_q^{K}  \right\}_{q=1}^{N_q}$, for cell $K$ to numerically calculate the streaming matrix:
...
In this case, this local cell-wise streaming matrix has dimensionality $(N_K \text{x} N_K \text{x} d)$
...
Finally, the last terms to consider of the discretized transport equation are those found on the faces of the cell boundary: $ {\Omega}_m \cdot  \left(  \vec{n} \psi_m, b_m  \right)_{\partial K}$. These terms are analagous to the cell mass matrix but are computed on the cell boundary with dimensionality $(d-1)$.
