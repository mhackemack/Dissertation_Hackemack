...
|| \Psi - \Psi_{exact} ||_{DG} = C \frac{h^q}{(p+1)^q},
\end{equation}
...
|| \Phi - \Phi_{exact} ||_{DG} = C \frac{h^q}{(p+1)^q},
\end{equation}
...
|| \Phi - \Phi_{exact} ||_{L_2} = C \frac{h^{q+1/2}}{(p+1)^q},
\end{equation}
...
Finally, we also seek to define the solution convergence rates in terms other than the maximum element diameter, $h$. For polytope meshes, this 
