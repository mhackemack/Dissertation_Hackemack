...
\subsubsection{Two-Grid Acceleration and a Variant}
...
\noindent where $\kappa$ is known as the penalty coefficient and $\kappa \gg 1$. In his work \cite{ref::nitsche_IP}, Nitsche further proposed a consistent formulation with this penalty method via symmetrization. This led to the following weak formulation of the Laplacian term with dirichlet boundary conditions,
...
\noindent where the face's unit normal direction, $\vec{n}$, has been arbitrarily chosen. The definition of Eq. (\ref{eq::solution_trace}) 
...
Besides Jacobi, SSOR and ILU preconditioning, we also seek to investigate multigrid (MG) methods to invert the MIP system matrix. 
