...
\noindent where we do not include the spatial parameter for clarity. If $\mathbb{D}$ is the diameter of the problem domain, then Eq. (\ref{eq::DSA_1G_simple_trans}) can slowly converge if the problem is optically thick ($ \sigma_t \, \mathbb{D} \gg 1$) and there is little absorption in the problem ($\sigma_s \approx \sigma_t$). Next, we define the scattering ratio, $c$, as the ratio between scattering and total cross sections: $c =\frac{\sigma_s}{\sigma_t}$. We insert this definition into Eq. (\ref{eq::DSA_1G_simple_trans}) to yield a simplified transport equation,

\begin{equation}
\label{eq::DSA_1G_simple_trans_withc}
\vec{\Omega} \cdot \vec{\nabla} \psi + \sigma_t \psi =  c \frac{ \sigma_t}{4 \pi} \phi + \frac{Q}{4 \pi}. 
\end{equation}

\noindent The Richardson method then calls for the following iterative approach where we use the half-iterate index, ${(k+1/2)}$,

\begin{equation}
\label{eq::DSA_1G_SI}
\vec{\Omega} \cdot \vec{\nabla} \psi^{(k+1/2)} + \sigma_t \psi^{(k+1/2)} =  c \frac{ \sigma_t}{4 \pi} \phi^{(k)} + \frac{Q}{4 \pi}. 
\end{equation}

\noindent We could iterate on Eq. (\ref{eq::DSA_1G_SI}) continuously until we arrive at a converged solution. Each iteration simply adds the contribution of the scattering source from the previous iteration into the total source term. However, as it was stated previously, this process can be prohibitively slow if the problem is optically thick with little absorption. 
...
\vec{\Omega} \cdot \vec{\nabla} \left( \psi - \psi^{(k+1/2)} \right) + \sigma_t \left( \psi -  \psi^{(k+1/2)}\right) =  c \frac{ \sigma_t}{4 \pi} \left( \phi -  \phi^{(k)}\right)  ,
\end{equation}

\noindent where we note that the distributed source, $Q$, vanishes. We can then add and subtract the term $c \frac{ \sigma_t}{4 \pi} \phi^{(k+1/2)}$ into the right-hand-side of Eq. (\ref{eq::DSA_1G_SI_err}) to form 

\begin{equation}
\label{eq::DSA_1G_SI_err_corrected}
\vec{\Omega} \cdot \vec{\nabla} \left( \psi - \psi^{(k+1/2)} \right) + \sigma_t \left( \psi -  \psi^{(k+1/2)}\right) =  c \frac{ \sigma_t}{4 \pi} \left( \phi -  \phi^{(k+1/2)}\right) + c \frac{ \sigma_t}{4 \pi} \left( \phi^{(k+1/2)} -  \phi^{(k)}\right)  
\end{equation}
...
\vec{\Omega} \cdot \vec{\nabla} \delta \psi^{(k+1/2)} + \sigma_t \delta \psi^{(k+1/2)} =  c \frac{ \sigma_t}{4 \pi} \delta \phi^{(k+1/2)}+ c \frac{ \sigma_t}{4 \pi} \left( \phi^{(k+1/2)} -  \phi^{(k)}\right)   .
\end{equation}
...
\noindent However, as stated in Section \ref{sec::DSA_Introduction_SA}, solving Eq. (\ref{eq::DSA_1G_SI_simple}) is just as difficult as the original transport equation. Therefore, we will form an approximate, low-order equation to Eq. (\ref{eq::DSA_1G_SI_simple}) that is easier to compute.

For this optically thick transport problem that is dominated by scattering, the diffusive error modes dominate \cite{ref::adams_larsen_iter_methods}. Therefore, our low-order approximation to Eq. (\ref{eq::DSA_1G_SI_simple}) needs to attenuate these diffusive modes. 




\subsection{THIS IS A DELIMITER!!!}
\label{sec::blah}
...
We have just provided a detailed description of the DSA methodology for a simple, 1-group, isotropic scattering transport problem.
...
\noindent We could solve these $G$ coupled multigroup diffusion equations of Eq. (\ref{eq::DSA_TG_diff_err_full}) for the iteration error. However, if the number of thermal groups becomes large, then these coupled equations could be become burdensome to solve. Instead, the TG method opts to perform a spectral collapse of the multigroup diffusion error. First, we factorize the multigroup error,

\begin{equation}
\label{eq::DSA_TG_fact_err}
\delta \phi_{g}^{(k+1/2)} = \xi_g \epsilon^{(k+1/2)} (\vec{r}), \qquad \sum_{g=1}^G \xi_g = 1,
\end{equation}
...
{\bf A} \epsilon^{(k+1/2)}  =  {\bf X} {\bf S_{U}} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right),
\end{equation}
...
\noindent The half-iterate solution, $\Phi^{(k+1/2)}$, is given by Eqs. (\ref{eq::DSA_TG_half_it_sol}) and (\ref{eq::DSA_TG_half_it_forcingfunc}), and the iteration error, $\delta \Phi^{(k+1/2)}$, is 

\begin{equation}
\label{eq::DSA_TG_proj_err}
\delta \Phi^{(k+1/2)} =  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right),
\end{equation}
...
\begin{aligned}
 \Phi^{(k+1)} &=  \left[ {\bf I} - {\bf D}{\bf L}^{-1} {\bf M} \left(  {\bf S_L} + {\bf S_D} \right) \right]^{-1} {\bf D}{\bf L}^{-1}  {\bf M} {\bf S_U} \Phi^{(k)} \\
  &+  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right) + {\bf b}
\end{aligned} .
\end{equation}
...
\begin{aligned}
 \Phi^{(k+1)} &=  {\bf F}  {\bf M} {\bf S_U} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right) + {\bf b} \\
&= {\bf F}  {\bf M} {\bf S_U} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \ {\bf F}  {\bf M} {\bf S_U} \Phi^{(k)} + {\bf b} - \Phi^{(k)}  \right) + {\bf b} \\
&= {\bf F}  {\bf M} {\bf S_U} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \ {\bf F}  {\bf M} {\bf S_U}  -  {\bf I} \right)\Phi^{(k)} + \left(  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}}  + {\bf I} \right) {\bf b} \\
&= \left[ {\bf F}  {\bf M} {\bf S_U} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}} \left(  \ {\bf F}  {\bf M} {\bf S_U}  -  {\bf I} \right) \right] \Phi^{(k)} + \left(  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S_{U}}  + {\bf I} \right) {\bf b}
\end{aligned} ,
\end{equation}
...
{\bf L} \Psi^{(k+1/2)} = {\bf M} {\bf S_L} \Phi^{(k+1/2)} + {\bf M}\left(  {\bf S_D} + {\bf S_U} \right)  \Phi^{(k)} + {\bf Q} .
\end{equation}
...
\Phi^{(k+1/2)} = \left[ {\bf I} - {\bf D}{\bf L}^{-1} {\bf M}{\bf S_L}\right]^{-1} {\bf D}{\bf L}^{-1}  {\bf M} \left(  {\bf S_D} + {\bf S_U} \right) \Phi^{(k)} + {\bf b} ,
\end{equation}
...
{\bf A} \epsilon^{(k+1/2)}  =  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right) \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right).
\end{equation}
...
\begin{aligned}
 \Phi^{(k+1)} =  &{\bf F}  {\bf M} \left( {\bf S_{D}} + {\bf S_{U}} \right) \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right) \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right) + {\bf b} \\
= &{\bf F}  {\bf M}\left( {\bf S_{D}} + {\bf S_{U}} \right) \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right) \left(  \ {\bf F}  {\bf M}\left( {\bf S_{D}} + {\bf S_{U}} \right)  -  {\bf I} \right)\Phi^{(k)} \\
&+ \left(  {\bf P} {\bf A}^{-1}  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right)  + {\bf I} \right) {\bf b} \\
= &\left[ {\bf F}  {\bf M} \left( {\bf S_{D}} + {\bf S_{U}} \right) +  {\bf P} {\bf A}^{-1}  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right) \left(  \ {\bf F}  {\bf M} \left( {\bf S_{D}} + {\bf S_{U}} \right)  -  {\bf I} \right) \right] \Phi^{(k)} \\
&+ \left(  {\bf P} {\bf A}^{-1}  {\bf X} \left( {\bf S_{D}} + {\bf S_{U}} \right)  + {\bf I} \right) {\bf b}
\end{aligned} ,
\end{equation}
...
\Phi^{(k+1/2)} = {\bf D}{\bf L}^{-1}  {\bf M} {\bf S} \Phi^{(k)} + {\bf b} ,
\end{equation}
...
\begin{equation}
\label{eq::DSA_MTG_1G_err_eq_op}
{\bf A} \epsilon^{(k+1/2)}  =  {\bf X} {\bf S} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right),
\end{equation}
...
\begin{aligned}
 \Phi^{(k+1)} &=  {\bf F}  {\bf M} {\bf S} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S} \left(  \Phi^{(k+1/2)} - \Phi^{(k)}  \right) + {\bf b} \\
&= {\bf F}  {\bf M} {\bf S} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S} \left(  \ {\bf F}  {\bf M} {\bf S} \Phi^{(k)} + {\bf b} - \Phi^{(k)}  \right) + {\bf b} \\
&= {\bf F}  {\bf M} {\bf S} \Phi^{(k)} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S} \left(  \ {\bf F}  {\bf M} {\bf S}  -  {\bf I} \right)\Phi^{(k)} + \left(  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S}  + {\bf I} \right) {\bf b} \\
&= \left[ {\bf F}  {\bf M} {\bf S} +  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S} \left(  \ {\bf F}  {\bf M} {\bf S}  -  {\bf I} \right) \right] \Phi^{(k)} + \left(  {\bf P} {\bf A}^{-1}  {\bf X} {\bf S}  + {\bf I} \right) {\bf b}
\end{aligned} ,
\end{equation}
...
\begin{aligned}
\Phi^{(k+1)} &= \left[ {\bf F} {\bf M} {\bf \Sigma} + {\bf P} {\bf A}^{-1}  {\bf X} {\bf \Sigma} \left(  {\bf F} {\bf M} {\bf \Sigma} - {\bf I} \right)  \right]   \Phi^{(k)} \\
